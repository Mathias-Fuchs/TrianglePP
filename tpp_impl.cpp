...
namespace tpp {
	...
	*/
	...
	/*! get the indices in the appended vertex list of the three vertices of the triangle pointed to by the face iterator
	*/
	std::tuple<int, int, int> Delaunay::fIterator::TriangleIndices(void)
	{
		Triwrap::__pmesh* tpmesh = (Triwrap::__pmesh*) this->MyDelaunay->m_pmesh;
		// get at the vloop pointer
		// a vloop pointer is also a Point*
		// so, get the address of the orig Point
		// a vertexptr can also serve as a Point*
		// so one can access the vertexptr just as one does access a vit.vloop

		// we draw inspiration from the following function
		//		int Delaunay::vertexId(vIterator const& vit) {
		//			Triwrap::__pmesh* tpmesh = (Triwrap::__pmesh*) vit.MyDelaunay->m_pmesh;
		//			return ((int*)vit.vloop)[tpmesh->vertexmarkindex];
		//		}
		Triwrap::__otriangle* ploop = (Triwrap::__otriangle*)(&(this->floop));
		Triwrap::vertex vertexptrOrig = (Triwrap::vertex)((ploop->tri)[plus1mod3[ploop->orient] + 3]);
		Triwrap::vertex vertexptrDest = (Triwrap::vertex)((ploop->tri)[minus1mod3[ploop->orient] + 3]);
		Triwrap::vertex vertexptrApex = (Triwrap::vertex)((ploop->tri)[ploop->orient + 3]);
		int indexOrig = ((int*)vertexptrOrig)[tpmesh->vertexmarkindex];
		int indexDest = ((int*)vertexptrDest)[tpmesh->vertexmarkindex];
		int indexApex = ((int*)vertexptrApex)[tpmesh->vertexmarkindex];
		return { indexOrig, indexDest, indexApex };
	}
	...
	*/
	...
	void Delaunay::WriteToObj(std::string filename)
	{
		std::ofstream out;
		out.open(filename, std::ios::out);
		int linenumber = 0;
		for (auto v = this->vbegin(); v != this->vend(); ++v) {
			tpp::Delaunay::Point l = *v;
			double x = l[0];
			double y = l[1];
			out << "v " << x << " " << y << " 0" << std::endl;
			int itsid = tpp::Delaunay::vertexId(v);
			std::cout << itsid + 1 << std::endl;
		}

		Point a;
		for (auto f = this->fbegin(); f != this->fend(); ++f) {
			auto vertexIndices = f.TriangleIndices();
			// the vertex index returned by Org(f, &a) is just -1 for inserted vertices, so we need to employ our own function.			

			out << "f " << 
				std::get<0>(vertexIndices) + 1 << " " <<
				std::get<1>(vertexIndices) + 1 << " " <<
				std::get<2>(vertexIndices) + 1 << " " <<
				std::endl;
		}
		out.close();
