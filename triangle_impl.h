...
#define encode(otri)                                                          \
  (triangle) ((int_ptr_type) (otri).tri | (int_ptr_type) (otri).orient)
...
/* Init an oriented triangle.                                                */

#define otriinit(otri1)														  \
  (otri1).tri = NULL;														  \
  (otri1).orient = 0
...
#define infect(otri)                                                          \
  (otri).tri[6] = (triangle)                                                  \
                    ((int_ptr_type) (otri).tri[6] | (int_ptr_type) 2l)
...
#define infected(otri)                                                        \
  (((int_ptr_type) (otri).tri[6] & (int_ptr_type) 2l) != 0l)
...
#define sdecode(sptr, osub)                                                   \
  (osub).ssorient = (int) ((int_ptr_type) (sptr) & (int_ptr_type) 1l);      \
  (osub).ss = (subseg *)                                                      \
              ((int_ptr_type) (sptr) & ~ (int_ptr_type) 3l)
...
#define sencode(osub)                                                         \
  (subseg) ((int_ptr_type) (osub).ss | (int_ptr_type) (osub).ssorient)
...
#define STARTINDEX 1
...

	{
		...
		alignptr = (int_ptr_type)(pool->nowblock + 1);
		/* Align the item on an `alignbytes'-byte boundary. */
		pool->nextitem = (VOID*)
			(alignptr + (int_ptr_type)pool->alignbytes -
			(alignptr % (int_ptr_type)pool->alignbytes));
...

	{
		...
		alignptr = (int_ptr_type)(pool->pathblock + 1);
...

	{
		...
		alignptr = (int_ptr_type)m->dummytribase;
		m->dummytri = (triangle*)
			(alignptr + (int_ptr_type)m->triangles.alignbytes -
			(alignptr % (int_ptr_type)m->triangles.alignbytes));
		...
		if (b->usesegments) {
			...
			alignptr = (int_ptr_type)m->dummysubbase;
			m->dummysub = (subseg*)
				(alignptr + (int_ptr_type)m->subsegs.alignbytes -
				(alignptr % (int_ptr_type)m->subsegs.alignbytes));
...

	{
		...
		alignptr = (int_ptr_type)(getblock + 1);
		foundvertex = (char*)(alignptr + (int_ptr_type)m->vertices.alignbytes -
			(alignptr % (int_ptr_type)m->vertices.alignbytes));
...
/*****************************************************************************/
  ...
  /*  cword = 4735; */
...

	{
		...
		if (encroached && (!b->nobisect || ((b->nobisect == 1) && (sides == 2)))) {
			...
			TRACE("  Queueing encroached subsegment: ");
...

	{
		...
		while (totalsamplesleft > 0) {
			...
			alignptr = (int_ptr_type)(sampleblock + 1);
			firsttri = (char*)(alignptr +
				(int_ptr_type)m->triangles.alignbytes -
				(alignptr %
				(int_ptr_type)m->triangles.alignbytes));
...

	{
		...
		while (m->lastflip != (struct flipstacker*) NULL) {
			...
			else if (m->lastflip->prevflip == (struct flipstacker*) - 1) { // ---> &insertvertex) {
...

	{
		...
		// added mrkkrj, 07.11.2011
		if (i < 2) {
			if (b->verbose) {
				printf("  Too little vertices left after duplicate vertex elimination! Breaking up.\n");
			}
			return 0;
		}
  ...
  /* Read the triangles from an .ele file. */
...

	{
		...
		TRACE(" -> splitencsegs ");
		TRACE2i(" m->steinerleft=", m->steinerleft);
...

	{
		...
		if (!b->quiet && b->conformdel && (m->badsubsegs.items > 0) &&
			...
			TRACE(" I ran out of Steiner points!!!");
		}
	}
...
#ifdef ANSI_DECLARATORS
	std::vector<double> vertices(struct mesh* m, struct behavior* b)
#else /* not ANSI_DECLARATORS */
	std::vector<double> vertices(m, b)
		struct mesh* m;
	struct behavior* b;
#endif /* not ANSI_DECLARATORS */

	{
		long outvertices = (b->jettison) ?
			m->vertices.items - m->undeads : m->vertices.items;
		std::vector<double> result;
		result.reserve(2 * outvertices);
		traversalinit(&m->vertices);
		vertex vertexloop = vertextraverse(m);
		while (vertexloop != (vertex)NULL) {
			if (!b->jettison || (vertextype(vertexloop) != UNDEADVERTEX)) {
				result.push_back(vertexloop[0]);
				result.push_back(vertexloop[1]);
			}
			vertexloop = vertextraverse(m);
		}
		return result;
...
#ifdef ANSI_DECLARATORS
	std::vector<int> triangles(struct mesh* m, struct behavior* b)
#else /* not ANSI_DECLARATORS */
	std::vector<int> edges(m, b)
		struct mesh* m;
	struct behavior* b;
#endif /* not ANSI_DECLARATORS */

	{
		...
		while (triangleloop.tri != (triangle*)NULL) {
			...
			dest(triangleloop, p2);
			apex(triangleloop, p3);
			int A = vertexmark(p1) - b->firstnumber;
			int B = vertexmark(p2) - b->firstnumber;
			int C = vertexmark(p3) - b->firstnumber;
			result.push_back(A);
			result.push_back(B);
			result.push_back(C);
			triangleloop.tri = triangletraverse(m);
